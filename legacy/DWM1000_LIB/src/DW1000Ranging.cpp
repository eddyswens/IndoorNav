...
void DW1000RangingClass::loop() {
	...
	if(_receivedAck) {
		...
		// Serial.print("Expected msg type: "); 
		...
		if(messageType == BLINK && _type == ANCHOR) {
			...
			if(addNetworkDevices(&myTag)) {
				...
				Serial.println("Sending Range Init");
			...
			if(_type == ANCHOR) {
				if(messageType != _expectedMsgId) {
					...
					// Serial.print("Unexpected message type. Expected: ");
					// Serial.println(_expectedMsgId);
				}
				if(messageType == POLL) {
					// Serial.println("I got POLL msg");
					...
					// Serial.println("I got RANGE msg");
...
void DW1000RangingClass::transmitRangingInit(DW1000Device* myDistantDevice) {
	...
	char hexCar[2];
		...
		// Serial.print("ranging_init data: ");
