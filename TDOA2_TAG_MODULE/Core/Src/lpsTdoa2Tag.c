...
static bool rxcallback(dwDevice_t *dev) {
  ...
  if (packet->type == PACKET_TYPE_TDOA2) {  // Если пакет типа TDOA2
    ...
    if (anchor < LOCODECK_NR_OF_TDOA2_ANCHORS) {  // Если айди меньше, чем количество анкеров
      uint32_t now_ms = HAL_GetTick();  // Текущее время в мс
...
static void sendTdoaToEstimatorCallback(tdoaMeasurement_t* tdoaMeasurement) {
  ...
  // estimatorEnqueueTDOA(tdoaMeasurement);  // Добавление в очередь эстиматора (локализатор) ОТПРАВКА ИЗМЕРЕНИЙ В ЭСТИМАТОР
...
static void Initialize(dwDevice_t *dev) {
  uint32_t now_ms = HAL_GetTick();  // Текущее время
...
static bool getAnchorPosition(const uint8_t anchorId, point_t* position) {
  ...
  uint32_t now_ms = HAL_GetTick();  // Получаем время сейчас
...
static uint8_t getActiveAnchorIdList(uint8_t unorderedAnchorList[], const int maxListSize) {
  uint32_t now_ms = HAL_GetTick();  // Время сейчас
